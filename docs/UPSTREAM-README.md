...
## Grok Bot mode (this fork)

This copy of JARVIS talks to a **Grok Bot** instead of Claude. Say "Jarvis, what's on today" and the words are typed into the open Grok Bot chat; when the bot answers, JARVIS reads the reply out in his voice and shows it on the HUD. JARVIS is the mouth and ears, the bot is the brain — there is no Claude turn in between, so the bot's answer is read word for word (markdown, links and code stripped for speech).

**How:** Grok Bot has no API, so the bridge drives the app's own window over Chrome's DevTools protocol. That needs Grok Bot launched with a control port, which `npm start` does for you (`open -a "Grok Bot" --args --remote-debugging-port=9333`; it quits and reopens Grok Bot if it was opened from the Dock). The port only listens on 127.0.0.1, and reopening Grok Bot from the Dock closes it.

**Voice commands handled locally:** "switch to Researcher" / "talk to Delivery" / "open Scout" changes bot (any name in the sidebar); "which bot" tells you who you're talking to.

**Long jobs and delegation:** the turn closes two seconds after the last thing the bot said ("Inbox is on it"), and anything the bot posts later, including answers relayed from other bots, is read out unprompted when it lands. After one of those late read-outs JARVIS goes back to waiting for his name rather than leaving the mic open. If nothing has come back after ~15 s he says "still on it"; at ~60 s he says he'll tell you when it lands.

**Ears:** hearing goes to Groq's free hosted Whisper (`GROQ_API_KEY`, console.groq.com, 8 hours of audio a day), with ElevenLabs Scribe as the automatic fallback if Groq is missing or errors, and the browser's own recogniser if neither key exists. Noise is filtered on the bridge: audio-event labels are stripped, Whisper's low-confidence output and its known noise-words ("you", "Bye.") are dropped. Tunables: `GROQ_STT_MODEL`, `GROQ_STT_LANGUAGE`, `GROQ_MIN_LOGPROB`.

**Settings** (`.env.local`): `JARVIS_BRAIN=grokbot|claude`, `GROKBOT_PORT=9333`, `GROKBOT_BOT=Chief of Staff`. `npm start -- --claude` (or `npm run start:claude`) runs the original Claude brain.
